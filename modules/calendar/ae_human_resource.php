...
// $Id: ae_resource.php 6048 2010-10-06 10:01:39Z ajdonnison $
	global $AppUI, $users, $event_id, $obj, $loadFromTab, $currentTabId;

// Make sure that we can see users that are allocated to the task.
	$q = new DBQuery;
	if ($event_id == 0) {
		// Add task creator to assigned users by default
		$assigned_perc = array($AppUI->user_id => array('contact_name' => $users[$AppUI->user_id]));
	} else {
		// Pull users on this task
		$q->addQuery("ut.user_id, concat_ws(' ', contact_last_name, contact_first_name) as contact_name");
		$q->addTable('user_events', 'ut');
		$q->leftJoin('users', 'u', array('user_id'));
		$q->leftJoin('contacts', 'c', 'c.contact_id = u.user_contact');
		$q->addWhere('event_id ='.$event_id.' AND event_id <> 0');
	...
	foreach ($assigned_perc as $user_id => $data) {
		$assigned[$user_id] = $data['contact_name'];
		$initPercAsignment .= "$user_id,";
	}
